...
- `/orders/schedule` — **Production schedule**. Every order from the sheet is a row. The left block is the same as the old spreadsheet: **ORDER, ITEM TYPE, CATEGORY, PRODUCT, PROVINCE, ORDER date (`07-Jan`), COURIER, WAYBILL, STATUS**. Those columns stay on screen while you scroll the calendar. Search by any of those fields. Amber rows have no **LD** or **LC** yet. Use **← Week / Week →** to jump, or scroll the dates; the week number stays frozen above the left block. Click a weekday cell to pick a code. `/schedule` redirects here.
