...
- `/orders` — order sheet (quote, client, prices, shop status) plus **Production schedule** at `/orders/schedule`. Click a row to view; **New order** or the pencil opens a form — you do not type in the cells. Rows stay one height (no wrapped description). An enquiry number is kept on the order when it came from Enquiries, but that column is not shown; the form says it is linked and can open the enquiry. Status and assigned operator are the same fields the floor writes.
...
- `/orders/schedule` — **Production schedule** (order list + week grid). Also under **Orders** as a tab. `/schedule` redirects here.
