...
Shop-floor and office app for Studio Delta (South Africa). The live host is **Railway** (Node/Express). Floor start / pause / resume / finish, durations, steel usage, and backboard usage save in the **same ORDERS table** as the office `/orders` page, on the Railway volume at `/app/data`. **Google Sheets is only a one-time import.**
...
## Host on Railway (local database)
...
1. In Railway → **New project** → **Deploy from GitHub** → this repo. Branch: `cursor/production-time-rules-ef3b`.
2. Variables:

   | Variable | Value |
   | --- | --- |
   | `TZ` | `Africa/Johannesburg` |
   | `SHEET_ID` | only needed for the **one-time** Google import |
   | `GOOGLE_SERVICE_ACCOUNT_JSON` | only needed for that import (and optional QC Drive/Gmail) |
   | `GMAIL_SENDER` | optional Workspace mailbox for QC PDFs / powder emails |

3. **Volumes** → add a volume mounted at **`/app/data`**. This file (`studio-delta.json`) is the database. Keep **one replica**.
4. **Settings → Networking → Generate domain**. Tablets open that HTTPS URL.

`GET /health` should return `"ok": true` and `"floorStore": "local"`. After the one-time import, `"localWorkbook": true`.

Login is still name + access code from the imported `Users` list.

### One-time Google Sheets import

Google is not the live database. Import once so Railway has Users, ORDERS, steel/backboard lists, and history:

1. Enable **Sheets** (and Drive/Docs only if you still want QC PDFs) on the service account.
2. Share the old production spreadsheet with that email as **Viewer** (Editor still works).
3. Set `SHEET_ID` and `GOOGLE_SERVICE_ACCOUNT_JSON`.
4. Open **Orders** → **Import from Sheets**, or let the first floor request import if `/app/data` is empty.
5. After that you can leave the Google variables in place for QC Drive, but **start / pause / resume / finish no longer write to Sheets**.

Local run (from `studio-delta-production/`):

```bash
cp .env.example .env   # Google vars optional after import
npm install
npm test
...
## Office pages (same ORDERS table)

Office **Orders** and the floor board share one `ORDERS` table in `/app/data`.

- `/orders` — quote, client, prices, **status**, assigned operator. Floor clocks, durations, steel usage, and backboard usage are stored on the same order records.
- `/schedule` — order list + week grid (Mon–Fri)
- `/debtors` — balances still owing

On Orders, **Import from Sheets** copies the spreadsheet **once** into the app. After that, edit in the app. Floor status changes show on `/orders` after refresh.
