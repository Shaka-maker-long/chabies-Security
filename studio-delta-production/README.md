...
| `DATA_DIR/paint-shop.json` | Paint shop send/receive batches, per-order costs, invoice ids |
| `DATA_DIR/paint-shop-invoices/` | Invoice files for received paint-shop batches |
| `DATA_DIR/glass-pos.json` | Glass purchase orders, receive batches, per-line costs |
| `DATA_DIR/glass-rates.json` | Glass rates per m² by type and thickness |
...
- `/planning` — **Planning**. Office page after Task times, with three tabs. **Planning** is the queue: tick **Not Yet Started** and **Ready for Steelwork** orders, assign people for each task except grinding, then **Schedule**. The app places each process in that person’s **next free paid slot** (07:45–12:00 and 12:30–15:45, Johannesburg). If you tick ten orders, assign the other work first; **grinding is auto-assigned afterwards** to the eligible person with the **most open time** (someone who would otherwise sit idle), among people ticked for **Grinding** plus **Profile cutting**, **Tagging**, or **Welding**. It does not squeeze into a gap between that person’s other jobs. **Journey** lists orders whose **start date** falls this week (order, product, start date, end date); click an order for that order’s visual journey (worker, task, work-day bars, Monday–Friday only). **People** is an Outlook-style week calendar for one person at a time, or **Compare with** a second person on the same grid. Drag a job later to push the work under it down; drag a later job to the top to swap. If you move profile cutting (or any earlier process), tagging, welding, grinding, paint, and assembly on the other people update with it. **Add other task** (or click an empty slot) places cleaning, a meeting, material, or any non-order job on that person and shifts later shop work. The calendar shades **Production Meeting** (07:45–08:00), **Lunch** (12:00–12:30), and **Cleaning** (15:15–15:45). Hours come from **Task times** for that product; a process with 0 or blank hours is skipped. Later orders queue behind earlier ones — the same person never has overlapping hours. Tagging waits for profile cutting. Welding waits for tagging and does **not** wait for plate cutting. Plate cutting also starts after tagging and may run next to welding. Grinding waits for that order’s welding, then fills an open slot. Frames then wait on the **Paint shop** lane for the earliest Monday on or after that grind, plus 5 calendar days, before assembly. Quality Control, Paint Preparation, and Painting are not booked here. This is not the shop **Schedule** and not the LD/LC production grid.
...
- **Ready for Assembly** can go to **Assembly** or **Paint Preparation**. Those are the two choices at that level. Painting is the next step after paint prep, not a choice on Ready for Assembly. Paint prep finish → **Ready for Painting** → Painting finish → **Ready for Assembly** so assembly can still happen. Assemblers start **Assemble**. Paint prep people start **Paint prep**. Admin (and anyone ticked for both tasks) sees both buttons.
...
- Painters need `Painting` on the Tasks column. They see **Ready for Painting**, not Ready for Assembly. Paint prep needs `Paint Preparation`. Assemblers need `Assembly`. Admin can start either path from Ready for Assembly.

## Worker schedule

Office sidebar → **Planning** has three tabs: plan the queue, this week’s starting orders (click for the visual journey), and an Outlook-style calendar per person (or two people compared). See `/planning` above.

Admin sidebar → **Schedule** (shop page) is the older floor board. Click Not Yet Started (and Ready for Steelwork) orders in the sequence they should be done, pick a process and a worker, then **Build calendar**. Block lengths come from past jobs of the same product and process (or a typical process time if that product is new). **Insert other task** (cleaning, meeting, etc.) pins a block on that worker’s calendar and pushes later work through 07:45–15:45, skipping lunch and weekends. This is a plan, not a live clock — the floor still starts/pauses/finishes as usual.
