...
- `/orders` — order sheet (quote, client, prices, shop status) plus **Production schedule** at `/orders/schedule` and **Delivery summary** at `/orders/delivery`. Click a row to view; **New order** or the pencil opens a form — you do not type in the cells. Rows stay one height (no wrapped description). An enquiry number is kept on the order when it came from Enquiries, but that column is not shown; the form says it is linked and can open the enquiry. Status and assigned operator are the same fields the floor writes. Every saved order is added to the production schedule automatically.
...
- `/orders/schedule` — **Production schedule**. Every order from the sheet is a row. Search by order number, product, or any left-column field. Amber rows have no **LD** or **LC** yet. The order-date column is `07-Jan`. Use **← Week / Week →** to jump, or scroll the calendar; the week number stays frozen with the order column. Click a weekday cell to pick a code. `/schedule` redirects here.
- `/orders/delivery` — **Delivery summary**. Lists orders that have **LD** or **LC** on the selected ISO week, with filters for category, day, and product/order search. Click a row to open that week on the schedule.
