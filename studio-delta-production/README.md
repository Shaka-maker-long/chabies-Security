...
- `/orders` — order sheet (quote, client, prices, shop status) plus **Production schedule** at `/orders/schedule`, **Delivery summary** at `/orders/delivery`, and **Job card** at `/orders/job-card`. Click a row to view; **New order** or the pencil opens a form — you do not type in the cells. Rows stay one height (no wrapped description). An enquiry number is kept on the order when it came from Enquiries, but that column is not shown; the form says it is linked and can open the enquiry. **Status is not typed on the form.** New orders start as **Not Yet Started**. Generating a job card moves that order to **Ready for Steelwork**. You can regenerate the PDF while the order is Ready for Steelwork or Profile Cutting; after that the shop floor owns status and the card cannot be generated again. **Printed job cards** stay on `/orders/job-card` — search, open, and print any generated PDF, even after the order has moved past Profile Cutting. The job card PDF has no Builder field and no BOM — Date start is the day the card was generated, and the cutting list is pasted the same way as the old Excel tool. Product images and dimensions are stored from the Studio Delta catalog. If Type is not Standard (Custom or New Design), a job card cannot be generated until someone confirms the sizes did not change or types the actual millimetres — catalog placeholders such as 1 × 1 × 1 are not accepted, and the confirmed sizes are written onto the order. Assigned operator is still the same field the floor writes. Every saved order is added to the production schedule automatically.
