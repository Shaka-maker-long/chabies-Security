...
- `/orders/schedule` — **Production schedule**. Every order from the sheet is a row. Mark weekday codes from the legend (LD Latest Delivery, LC Latest Courier, and the production stages). Courier and waybill can be typed here; the other left columns come from the order. `/schedule` redirects here.
