...
- `/enquiries` — Enquiry log (view-only sheet) plus **Dashboard** at `/enquiries/dashboard`. The dashboard is a weekly or monthly line of enquiry and quote **counts**, switchable to **revenue** (quoted vs ordered excl VAT). A **Month** filter (or 6 months / this year / all time) applies to every chart, including funnel and pipeline. Click a line-graph dot (or a bar) to list the enquiries in that slice, then click a row for the full enquiry popup. Also funnel, live pipeline, win/loss, time-to-order, stuck work, source, type, stage time, workload, money, CATERGORY, product, province, and Outlook (Johannesburg dates). On the sheet, numbers start at `#1996` and count up. **New enquiry** and each row’s **Edit** button open the same popup — you do not type in the cells. **MONTH ENQUIRED** fills from **DATE ENQUIRED**. Columns through **CLIENT NAME** stay frozen while you scroll. Capture product names here. **Custom** asks whether the change is **Dimensions**, **Colour**, or **Other** (Other names are saved onto the dropdown). **New Design** needs a full description. **STATUS is not a free dropdown** — it moves when the assigned office person saves a deliverable (cost sheet, approval, quote PDF, follow-up screenshot, POP, drawing). **OPENED** and **LIFESPAN** (Johannesburg time) track capture through order; **STATUS · update** shows the full timestamped timeline. Costing asks for an optional approver and a **quoting person**. Open **STATUS · update** on any enquiry to see **Files**: Outlook emails, cost sheet, quote PDF, follow-up screenshots, proof of payment, and drawing stay listed and openable at any time. **CORRESPONDANCE**: drag the email from the classic Outlook message list onto **Drop the Outlook email here**, wait for the subject, then **Save update**. **Open in Outlook** downloads the saved `.msg` so Windows can open classic Outlook (not Outlook (new)). Product **values, delivery excl VAT, and quotation number** are entered when the quote PDF is uploaded. The quotation number defaults to the next `SOQ` after the last used number (enquiries and orders) and cannot be a duplicate.
