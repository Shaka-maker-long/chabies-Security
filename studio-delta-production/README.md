...
- `/planning` — **Planning**. Tick **Not Yet Started** and **Ready for Steelwork** orders, assign the people ticked for each task on Users, and the app places each process in that person’s **next free paid slot** (07:45–12:00 and 12:30–15:45, Johannesburg). **Journey** is the whole order as a grid: order number, worker, task, then work-day dates (Monday–Friday only) with a bar on the days that job sits. **People** is every worker’s week, one row each, same work days. Hours come from **Task times** for that product; a process with 0 or blank hours is skipped. Later orders queue behind earlier ones — the same person never has overlapping hours. Tagging waits for profile cutting. Welding waits for tagging and does **not** wait for plate cutting. Plate cutting also starts after tagging and may run next to welding. Grinding waits for welding. Frames then wait on the **Paint shop** lane for the earliest Monday on or after that grind, plus 5 calendar days, before assembly. Quality Control, Paint Preparation, and Painting are not booked here. This is not the shop **Schedule** and not the LD/LC production grid.
