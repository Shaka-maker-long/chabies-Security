...
- `/orders/schedule` — **Production schedule**. Every order from the sheet is a row. Click a weekday cell to pick a code (LD Latest Delivery, LC Latest Courier, and the production stages) — the grid does not put a dropdown in every cell. Courier and waybill can be typed here; the other left columns come from the order. `/schedule` redirects here.
