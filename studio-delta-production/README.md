...
- `/planning` — **Planning**. Tick **Not Yet Started** and **Ready for Steelwork** orders, assign the people ticked for each task on Users except **grinding**, and the app places each process in that person’s **next free paid slot** (07:45–12:00 and 12:30–15:45, Johannesburg). If you tick ten orders, assign the other work first; **grinding is auto-assigned afterwards** to the eligible person with the **most open time** (someone who would otherwise sit idle), among people ticked for **Grinding** plus **Profile cutting**, **Tagging**, or **Welding**. It does not squeeze into a gap between that person’s other jobs. **Journey** is the whole order as a grid: order number, worker, task, then work-day dates (Monday–Friday only) with a bar on the days that job sits. **People** is every worker’s week, one row each, same work days. Hours come from **Task times** for that product; a process with 0 or blank hours is skipped. Later orders queue behind earlier ones — the same person never has overlapping hours. Tagging waits for profile cutting. Welding waits for tagging and does **not** wait for plate cutting. Plate cutting also starts after tagging and may run next to welding. Grinding waits for that order’s welding, then fills an open slot. Frames then wait on the **Paint shop** lane for the earliest Monday on or after that grind, plus 5 calendar days, before assembly. Quality Control, Paint Preparation, and Painting are not booked here. This is not the shop **Schedule** and not the LD/LC production grid.
...
Office sidebar → **Planning** is the order journey grid and every worker’s week (work days only), hours from Task times. See `/planning` above.
