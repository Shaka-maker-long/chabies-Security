...
assert.ok(page.indexOf("/orders/job-card/existing") !== -1);
assert.ok(page.indexOf("Existing job cards") !== -1);
assert.ok(page.indexOf("Printed job cards") === -1, "printed list is a separate subpage");
...
assert.ok(page.indexOf("Open PDF") !== -1);
...
const existingPage = fs.readFileSync(path.join(__dirname, "../public/job-card-existing.html"), "utf8");
assert.ok(existingPage.indexOf("Existing job cards") !== -1);
assert.ok(existingPage.indexOf("Open / print") !== -1);
assert.ok(existingPage.indexOf("/orders/job-card") !== -1);
assert.ok(existingPage.indexOf("Generate again") !== -1);
assert.ok(existingPage.indexOf("/api/office/job-cards") !== -1);
