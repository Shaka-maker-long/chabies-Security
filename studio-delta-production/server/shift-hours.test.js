...
(async function main() {
  ...
  const stillWorking = await callShopFunction("facilityShiftState", [new Date("2026-09-14T15:20:00+02:00")]);
  assert.strictEqual(stillWorking.id, "working", "cleaning must not start before 15:45: " + JSON.stringify(stillWorking));
  const lateWorking = await callShopFunction("facilityShiftState", [new Date("2026-09-14T15:44:00+02:00")]);
  assert.strictEqual(lateWorking.id, "working", JSON.stringify(lateWorking));
  const cleaning = await callShopFunction("facilityShiftState", [new Date("2026-09-14T15:50:00+02:00")]);
