...
assert.ok(js.indexOf("Inventory Management") !== -1, "inventory is its own office page");
assert.ok(js.indexOf("/inventory") !== -1);
...
assert.ok(tasksHtml.indexOf("order_label") !== -1, "drawing cards show order numbers so Erin can find S260246");
assert.ok(tasksHtml.indexOf(">Drawing<") !== -1);
assert.ok(js.indexOf("sdIsDrawingOwner") !== -1, "Erin can open My tasks for drawings");
...
assert.ok(floor.indexOf("is-meeting") !== -1);
assert.ok(floor.indexOf("is-cleaning") !== -1);
assert.ok(floor.indexOf("is-end") !== -1);
assert.ok(floor.indexOf("getWorkerDayWork") !== -1);
assert.ok(floor.indexOf("function facilityShiftState") !== -1);
assert.ok(floor.indexOf("Production Meeting") !== -1);
assert.ok(floor.indexOf("End of shift") !== -1);
assert.ok(floor.indexOf("No shop work recorded today.") !== -1);
...
assert.ok(floor.indexOf("p === 'profile cutting'") !== -1);
assert.ok(floor.indexOf("p === 'plate cutting'") !== -1);
assert.ok(floor.indexOf("return 'plate'") !== -1, "plate cutting is a sheet and grinder, not the tube saw");
assert.ok(floor.indexOf("twin-cabinet") !== -1, "weld, tag, grind, and assembly show a cabinet");
assert.ok(floor.indexOf("twin-glass") !== -1, "assembly fits glass into the cabinet");
assert.ok(floor.indexOf("twin-tube") !== -1, "profile cutting shows a steel tube");
assert.ok(floor.indexOf("twin-sheet") !== -1, "plate cutting shows a steel sheet");
assert.ok(floor.indexOf("twin-chop") !== -1, "profile cutting uses a round saw");
...
assert.ok(floor.indexOf("QC photos did not land on the PDF") !== -1, "tablet must not treat a photo-less QC PDF as finished");
...
assert.ok(processJs.indexOf("Erin uploads the drawing") !== -1, "drawing tasks always go to Erin");
assert.ok(!/complete_order[\s\S]{0,400}Drawing assigned to/.test(processJs), "POP must not ask who uploads the drawing");
...
assert.ok(enquiriesHtml.indexOf("Is there enough information to move this to costing?") !== -1);
assert.ok(enquiriesHtml.indexOf("A product name is not enough") !== -1);
assert.ok(enquiriesHtml.indexOf("what they want customised") !== -1);
...
assert.ok(processJs.indexOf("complete_chase") !== -1);
assert.ok(processJs.indexOf("assign_waiting") !== -1);
assert.ok(pipelineJs.indexOf("function applyManualStatus") !== -1);
assert.ok(pipelineJs.indexOf("function readyToCost") !== -1);
...
assert.ok(enquiriesHtml.indexOf("data-delete-enquiry") === -1, "enquiry sheet must not delete a row");
assert.ok(enquiriesHtml.indexOf("Delete enquiry") === -1);
assert.ok(enquiriesHtml.indexOf("bi-trash") === -1);
assert.ok(enquiriesHtml.indexOf("sdConfirmDelete") === -1);
assert.ok(processJs.indexOf("sdConfirmDelete") === -1);
assert.ok(processJs.indexOf("data-delete-file") === -1, "files list must not delete an attachment");
assert.ok(processJs.indexOf("Delete enquiry") === -1);
assert.ok(processJs.indexOf("sdDeleteConfirm") === -1);
assert.ok(processJs.indexOf("Issue another quote") !== -1 || officeJs.indexOf("Issue another quote") !== -1 || processJs.indexOf("complete_quote") !== -1);
assert.ok(processJs.indexOf("Save Correspondance link") !== -1 || processJs.indexOf("add_correspondence") !== -1);
assert.ok(officeJs.indexOf("/files/:kind") !== -1);
assert.ok(officeJs.indexOf("deleteEnquiryAttachment") === -1);
assert.ok(officeJs.indexOf("app.delete(\"/api/office/enquiries/:enquiryNo\"") === -1);
...
assert.ok(enquiriesHtml.indexOf("/enquiry-process.js?v=drawing-erin-tasks") !== -1);
assert.ok(enquiriesHtml.indexOf("Erin uploads the drawing") !== -1);
assert.ok(enquiriesHtml.indexOf("ob_drawing_who") === -1, "drawing assignee stays Erin");
...
assert.ok(ordersHtml.indexOf("/orders/consumables") === -1, "consumables is not an Orders tab");
assert.ok(ordersHtml.indexOf(">Consumables<") === -1, "Consumables tab is gone from Orders");
assert.ok(indexJs.indexOf('"/inventory"') !== -1 || indexJs.indexOf("/inventory") !== -1);
assert.ok(indexJs.indexOf('res.redirect(302, "/inventory")') !== -1);
assert.ok(officeJs.indexOf("/api/office/consumables") !== -1);
assert.ok(officeJs.indexOf("/api/office/consumables/use") !== -1);
assert.ok(officeJs.indexOf("/api/office/consumables/receive") !== -1);
assert.ok(officeJs.indexOf("/api/office/consumables/purchases") !== -1);
const inventoryHtml = fs.readFileSync(path.join(__dirname, "../public/inventory.html"), "utf8");
assert.ok(inventoryHtml.indexOf("Inventory Management") !== -1);
assert.ok(inventoryHtml.indexOf('href="/inventory">Consumables<') !== -1);
assert.ok(inventoryHtml.indexOf('href="/inventory/purchases">Purchase orders<') !== -1);
assert.ok(inventoryHtml.indexOf('href="/inventory/steel">Steel<') !== -1);
assert.ok(inventoryHtml.indexOf('href="/inventory/glass">Glass<') !== -1);
assert.ok(inventoryHtml.indexOf("/inventory/wood") === -1, "wood is not an inventory tab");
assert.ok(inventoryHtml.indexOf("Current Stock") !== -1);
assert.ok(inventoryHtml.indexOf("Ordered Stock") !== -1);
assert.ok(inventoryHtml.indexOf(">ROP<") !== -1 || inventoryHtml.indexOf(">ROP</") !== -1 || inventoryHtml.indexOf("ROP</th>") !== -1);
assert.ok(inventoryHtml.indexOf("Unit Price") !== -1);
assert.ok(inventoryHtml.indexOf("Total Value") !== -1);
assert.ok(inventoryHtml.indexOf("Log usage") !== -1);
assert.ok(inventoryHtml.indexOf("Create purchase order") !== -1);
assert.ok(inventoryHtml.indexOf("useWorker") !== -1);
assert.ok(inventoryHtml.indexOf("useLines") !== -1);
assert.ok(inventoryHtml.indexOf("Add another item") !== -1);
assert.ok(inventoryHtml.indexOf("mobile-use") !== -1);
assert.ok(inventoryHtml.indexOf("addUseLine") !== -1);
assert.ok(inventoryHtml.indexOf("General") !== -1);
assert.ok(inventoryHtml.indexOf("Generate purchase order") !== -1);
assert.ok(inventoryHtml.indexOf("Open PDF") !== -1);
assert.ok(inventoryHtml.indexOf("sdRequireOffice(\"inventory\")") !== -1);
assert.ok(inventoryHtml.indexOf("Steel stock") === -1);
assert.ok(inventoryHtml.indexOf("Glass stock") === -1);
assert.ok(inventoryHtml.indexOf("Wood stock") === -1);
assert.ok(inventoryHtml.indexOf("Hinges, bullets, screws") !== -1);
assert.ok(inventoryHtml.indexOf("Orders views") === -1, "inventory is not an Orders tab bar");
assert.ok(indexJs.indexOf('app.get("/inventory/steel"') !== -1);
assert.ok(indexJs.indexOf('app.get("/inventory/glass"') !== -1);
assert.ok(indexJs.indexOf('app.get("/inventory/purchases"') !== -1);
assert.ok(officeJs.indexOf("/api/office/inventory/steel") !== -1);
assert.ok(officeJs.indexOf("/api/office/inventory/glass") !== -1);
assert.ok(officeJs.indexOf("/api/office/consumables/purchases/:id/pdf") !== -1);
assert.ok(floor.indexOf('id="link-inventory"') !== -1);
assert.ok(floor.indexOf("Inventory Management") !== -1);
assert.ok(ordersHtml.indexOf("/orders/paint-shop") !== -1);
assert.ok(ordersHtml.indexOf("Paint shop") !== -1);
assert.ok(ordersHtml.indexOf("/orders/dashboard") !== -1);
assert.ok(ordersHtml.indexOf("Dashboard") !== -1);
assert.ok(indexJs.indexOf("/orders/dashboard") !== -1);
assert.ok(officeJs.indexOf("/api/office/orders/dashboard") !== -1);
...
assert.ok(ordersDashHtml.indexOf("Shop pipeline") !== -1);
assert.ok(ordersDashHtml.indexOf("Stuck / ageing") !== -1);
assert.ok(ordersDashHtml.indexOf("Delivery this week and next") !== -1);
assert.ok(ordersDashHtml.indexOf("Throughput") !== -1);
assert.ok(ordersDashHtml.indexOf("Blockers") !== -1);
assert.ok(ordersDashHtml.indexOf("chart.js") !== -1);
assert.ok(ordersDashHtml.indexOf("sdOfficeFetch") !== -1);
assert.ok(ordersHtml.indexOf("URLSearchParams(location.search).get(\"q\")") !== -1);
...
assert.ok(ordersHtml.indexOf("id=\"f_correct_status\"") !== -1);
assert.ok(ordersHtml.indexOf("Correct shop status") !== -1);
assert.ok(ordersHtml.indexOf("/api/office/orders/correct-status") !== -1);
assert.ok(ordersHtml.indexOf("/orders/correct") !== -1);
assert.ok(ordersHtml.indexOf("Fix status") !== -1);
assert.ok(officeJs.indexOf("/api/office/orders/correct-status") !== -1);
assert.ok(officeJs.indexOf("correctOrderShop") !== -1);
assert.ok(officeJs.indexOf("correctManyOrders") !== -1);
const correctHtml = fs.readFileSync(path.join(__dirname, "../public/orders-correct.html"), "utf8");
assert.ok(correctHtml.indexOf("Fix status") !== -1);
assert.ok(correctHtml.indexOf("stays clocked") !== -1);
assert.ok(correctHtml.indexOf("/api/office/orders/correct-status") !== -1);
assert.ok(indexJs.indexOf("/orders/correct") !== -1);
assert.ok(ordersHtml.indexOf("Mark selected as important") !== -1);
assert.ok(ordersHtml.indexOf("Clear all orders") !== -1);
assert.ok(ordersHtml.indexOf("Remove this order") !== -1);
assert.ok(ordersHtml.indexOf("viewRemoveBtn") !== -1);
assert.ok(ordersHtml.indexOf("method: \"DELETE\"") !== -1);
...
assert.ok(ordersHtml.indexOf("has-comment") !== -1, "open comments marked on cells");
assert.ok(ordersHtml.indexOf("position:relative") === -1 || ordersHtml.indexOf("td.cell-commentable { position:relative") === -1, "comment CSS must not break sticky freeze");
...
assert.ok(ordersHtml.indexOf("Add to Orders") !== -1);
assert.ok(ordersHtml.indexOf("Add existing order") !== -1);
assert.ok(ordersHtml.indexOf("/orders/onboard") !== -1);
assert.ok(ordersHtml.indexOf("/orders/new") !== -1, "New order opens the quantity / extra-item split form");
assert.ok(ordersHtml.indexOf("location.href='/orders/new'") !== -1);
assert.ok(ordersHtml.indexOf("Paste orders") !== -1);
assert.ok(ordersHtml.indexOf("/orders/paste") !== -1);
assert.ok(ordersHtml.indexOf("id=\"pasteBox\"") === -1, "paste box must not sit on the Orders sheet");
assert.ok(ordersHtml.indexOf("id=\"pasteText\"") === -1, "Orders sheet stays the order grid");
assert.ok(ordersHtml.indexOf("order-paste-ui.js") === -1);
assert.ok(ordersHtml.indexOf("Import from Sheets") === -1, "Orders must not import from Google Sheets");
const pasteHtml = fs.readFileSync(path.join(__dirname, "../public/orders-paste.html"), "utf8");
assert.ok(pasteHtml.indexOf("Paste orders") !== -1);
assert.ok(pasteHtml.indexOf("id=\"pasteText\"") !== -1);
assert.ok(pasteHtml.indexOf("order-paste-ui.js") !== -1);
assert.ok(pasteHtml.indexOf("Everyone has paid in full") !== -1);
assert.ok(pasteHtml.indexOf("Design Fee and other fee rows are skipped") !== -1);
assert.ok(pasteHtml.indexOf("Import from Sheets") === -1);
...
assert.ok(onboardHtml.indexOf("/api/office/orders/onboard") !== -1);
assert.ok(officeJs.indexOf("onboardExistingOrder") !== -1);
assert.ok(officeJs.indexOf("pasteOrdersFromSheet") !== -1);
assert.ok(indexJs.indexOf("/orders/onboard") !== -1);
assert.ok(indexJs.indexOf("orders-paste.html") !== -1);
assert.ok(ordersHtml.indexOf("renderReadyEnquiries") !== -1);
assert.ok(officeJs.indexOf("readyEnquiries") !== -1);
assert.ok(officeJs.indexOf("listEnquiriesWaitingForOrders") !== -1);
...
assert.ok(scheduleHtml.indexOf("/orders/dashboard") !== -1);
...
assert.ok(scheduleHtml.indexOf("order_date_label") !== -1);
assert.ok(scheduleHtml.indexOf('placeholder="07-Jan"') === -1, "schedule ORDER date is not typed");
assert.ok(scheduleHtml.indexOf('["order_date","ORDER", true') !== -1, "schedule ORDER date is locked to the payment date");
assert.ok(scheduleHtml.indexOf("moveReason") !== -1);
assert.ok(scheduleHtml.indexOf("LD*") !== -1);
assert.ok(scheduleHtml.indexOf("Give a reason") !== -1 || scheduleHtml.indexOf("Type why it moved") !== -1);
...
assert.ok(indexJs.indexOf("/orders/new") !== -1);
...
assert.ok(fromEnquiryHtml.indexOf("Add another item") !== -1);
assert.ok(fromEnquiryHtml.indexOf("/api/office/orders/create") !== -1);
assert.ok(fromEnquiryHtml.indexOf("Amount paid (Incl VAT)") !== -1);
assert.ok(fromEnquiryHtml.indexOf("function isFeeLine") !== -1);
assert.ok(fromEnquiryHtml.indexOf("Design Fee") !== -1);
...
assert.ok(floor.indexOf("confirmPowderColour") !== -1);
assert.ok(floor.indexOf("Powder coating colour") !== -1);
assert.ok(floor.indexOf("Confirm the powder coating colour") !== -1);
assert.ok(toOrderHtml.indexOf("/orders/glass-rates") !== -1);
assert.ok(toOrderHtml.indexOf("/orders/consumables") === -1);
...
assert.ok(costHtml.indexOf("/orders/dashboard") !== -1);
assert.ok(costHtml.indexOf("/orders/consumables") === -1);
assert.ok(costHtml.indexOf("Production matrix") !== -1);
assert.ok(costHtml.indexOf("cost-matrix") !== -1);
assert.ok(costHtml.indexOf("id=\"productShot\"") !== -1, "hover the product name for the catalog photo");
assert.ok(costHtml.indexOf("data-product-shot") !== -1);
assert.ok(costHtml.indexOf("col-product tooltip") === -1, "product photo is not a centered overlay that covers the matrix");
assert.ok(costHtml.indexOf("function showProductShot") !== -1);
assert.ok(costHtml.indexOf("sits beside the name") !== -1);
...
assert.ok(costHtml.indexOf("Steel cost") === -1, "the materials column is not named Steel");
assert.ok(costHtml.indexOf(">Materials<") !== -1);
assert.ok(costHtml.indexOf("(Lab+Materials)/Sale") !== -1);
assert.ok(costHtml.indexOf("col-money\">Steel<") === -1);
assert.ok(costHtml.indexOf("col-money\">Glass<") === -1);
assert.ok(costHtml.indexOf("col-money\">Powder<") === -1);
assert.ok(costHtml.indexOf("Glass actual") === -1, "glass is inside Materials, not its own column");
assert.ok(costHtml.indexOf("Powder actual") === -1, "powder is inside Materials, not its own column");
...
assert.ok(costHtml.indexOf("per person") !== -1);
assert.ok(costHtml.indexOf("not the workstation") !== -1);
assert.ok(costHtml.indexOf("that person's hourly rate") !== -1);
assert.ok(costHtml.indexOf("value=\"all\" selected") !== -1, "Cost defaults to all time so finished work is not hidden");
assert.ok(costHtml.indexOf("Cost is live") !== -1);
assert.ok(costHtml.indexOf("Africa/Johannesburg") !== -1);
assert.ok(costHtml.indexOf("Open Rates") !== -1);
...
assert.ok(officeJs.indexOf("/api/office/paint-shop/adopt") !== -1);
...
assert.ok(paintShopHtml.indexOf("Paint Shop") !== -1);
assert.ok(paintShopHtml.indexOf("powder coaters") !== -1);
assert.ok(paintShopHtml.indexOf("Saved as Not Yet Started") !== -1);
assert.ok(paintShopHtml.indexOf("They are at the paint shop") !== -1);
...
assert.ok(floor.indexOf("id=\"noPlateModal\"") !== -1, "NO PLATES must ask in a modal");
assert.ok(floor.indexOf("You are about to mark this item as no plate.") !== -1);
assert.ok(floor.indexOf("id=\"noPlateProceedBtn\">Proceed") !== -1);
assert.ok(floor.indexOf("id=\"noPlateCancelBtn\"") !== -1);
assert.ok(floor.indexOf("function proceedMarkNoPlates") !== -1);
assert.ok(floor.indexOf("has no plates? It will leave the plate cutting list") === -1, "native confirm is too easy to tap through");
assert.ok(floor.indexOf("qcKey === 'Assembly' || qcKey === 'Final QC'") === -1, "backboard size is not on Final QC");
assert.ok(floor.indexOf("if (qcKey === 'Assembly')") !== -1, "backboard size is logged when assembly finishes");
...
assert.ok(floor.indexOf("function adminIsLookOnlyOnBoard") !== -1, "admins with QC access can clock QC");
assert.ok(floor.indexOf("function hasQcFloorAccess") !== -1, "QC access is the Users Quality Control tick");
assert.ok(floor.indexOf("adminIsLookOnlyOnBoard()") !== -1, "ADMIN VIEW ONLY is not forced on every admin");
assert.ok(floor.indexOf("session.role === 'Quality Control' && hasQcFloorAccess()") !== -1, "QC-ticked admins work the QC board");
assert.ok(floor.indexOf("Admin is look-only except Quality Control") !== -1);
...
assert.ok(codeGs.indexOf("return r === \"assembly\" || p === \"assembly\";") !== -1, "backboard size is required on assembly finish");
...
assert.ok(codeGs.indexOf("function countQcPhotoFiles_") !== -1, "finish QC counts photos before closing the clock");
assert.ok(codeGs.indexOf("function reopenQcAfterPdfFail") !== -1, "QC clock reopens if the PDF cannot take the photos");
assert.ok(codeGs.indexOf("queuedCount < 1 || !queuedPhotos.length") !== -1, "Drive queue must not build a photo-less QC PDF");
...
assert.ok(codeGs.indexOf("function workerCanViewRole") !== -1, "admins can still open look-only boards");
assert.ok(codeGs.indexOf("if (profile.isAdmin) return true;") !== -1);
assert.ok(codeGs.indexOf("tasks: KNOWN_FLOOR_TASKS.slice()") === -1, "admin login must not grant every shop task");
assert.ok(codeGs.indexOf("extra || (isAdmin ? \"\" : role)") !== -1, "admin Tasks ticks are the floor assignment");
...
assert.ok(codeGs.indexOf("function userOverseesFloor") !== -1);
assert.ok(codeGs.indexOf("userOverseesFloor(want)") !== -1);
assert.ok(codeGs.indexOf("If it is Other, specify") !== -1);
assert.ok(floor.indexOf("sessionSeesIdleAlerts") !== -1);
assert.ok(floor.indexOf("canSeeIdleAlerts") !== -1);
assert.ok(floor.indexOf("idle-only") !== -1);
assert.ok(floor.indexOf("15 minutes") !== -1);
assert.ok(floor.indexOf("If it is Other, specify what they did") !== -1);
assert.ok(floor.indexOf("withFailureHandler") !== -1 && floor.indexOf("assignIndirectTask") !== -1, "Assign task must show a failure");
assert.ok(floor.indexOf("window.assignIdleTask") !== -1, "Assign task stays callable from the button");
assert.ok(floor.indexOf("onclick=\"assignIdleTask") === -1, "Assign task must not use a broken inline onclick");
assert.ok(floor.indexOf("data-idle-row") !== -1 && floor.indexOf("closest('[data-idle-assign]')") !== -1, "Assign task uses a click listener on the button");
assert.ok(floor.indexOf('type="button"') !== -1 && floor.indexOf("data-idle-assign") !== -1, "Assign task is a button, not a form submit");
...
assert.ok(planningHtml.indexOf("orderId: \"Other\"") !== -1);
assert.ok(planningHtml.indexOf("openOtherDetail") !== -1);
...
assert.ok(floor.indexOf("/ar-measure.js") !== -1, "floor loads phone AR measure");
assert.ok(floor.indexOf("openQcArMeasure") !== -1);
assert.ok(floor.indexOf("Overall size check (phone AR)") !== -1);
const arMeasureJs = fs.readFileSync(path.join(__dirname, "../public/ar-measure.js"), "utf8");
assert.ok(arMeasureJs.indexOf("immersive-ar") !== -1);
assert.ok(arMeasureJs.indexOf("sdOpenArMeasure") !== -1);
...
assert.ok(jobCardHtml.indexOf("/orders/dashboard") !== -1);
...
assert.ok(deliveryHtml.indexOf("/orders/dashboard") !== -1);
...
assert.ok(repliesHtml.indexOf("/sd-pwa.js?v=pwa-stg1") !== -1);
...
assert.ok(dashHtml.indexOf("/sd-pwa.js?v=pwa-stg1") !== -1, "dashboard must boot the PWA");
...
assert.ok(gasJs.indexOf("reopenQcAfterPdfFail") !== -1, "PDF save failure must reopen the QC job");
...
assert.ok(usersBackupHtml.indexOf("moved into a Shared drive") !== -1, "Backup page must say to move the existing folder");
...
assert.ok(floor.indexOf("/sd-pwa.js?v=pwa-stg1") !== -1);
