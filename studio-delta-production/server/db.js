...
function closeReasonOf(row) {
  const fromOutcome = String((row && row.client_outcome && row.client_outcome.reason) || "").trim();
  if (fromOutcome) return fromOutcome;
  const ev = (Array.isArray(row && row.events) ? row.events : []).filter((e) =>
    e && (e.kind === "complete_reject" || e.kind === "close")
  ).slice(-1)[0];
...
function unlinkEnquiryStoredFile(enquiryNo, storedAs) {
  const name = String(storedAs || "").trim();
  if (!name) return;
  try {
    const file = path.join(enquiryFilesDir(enquiryNo), name);
    if (fs.existsSync(file)) fs.unlinkSync(file);
  } catch (e) {}
}

function refreshCanonicalQuotePdf(row) {
  const quotes = Array.isArray(row.quotes) ? row.quotes : [];
  const last = quotes.length ? quotes[quotes.length - 1] : null;
  if (!last) {
    removeEnquiryQuotePdf(row.enquiry_no);
    row.quote_no = "";
    row.date_quoted = "";
    row.quote_pdf_name = "";
    row.quote_pdf_uploaded_at = "";
    return;
  }
  row.quote_no = last.quote_no || "";
  row.date_quoted = last.date_quoted || "";
  row.quote_pdf_name = (last.file && last.file.filename) || row.quote_pdf_name || "quote.pdf";
  row.quote_pdf_uploaded_at = last.uploaded_at || last.date_quoted || row.quote_pdf_uploaded_at || "";
  const kind = (last.file && last.file.kind) || (last.n ? "quote_" + last.n : "");
  const stored = kind ? readEnquiryAttachment(row.enquiry_no, kind) : null;
  if (stored && stored.buffer) {
    try { fs.writeFileSync(enquiryQuotePdfPath(row.enquiry_no), stored.buffer); } catch (e) {}
  }
}

function deleteEnquiryAttachment(enquiryNo, kind) {
  const raw = getEnquiryRaw(enquiryNo);
  if (!raw) throw new Error("Enquiry not found");
  const want = String(kind || "").trim();
  if (!want) throw new Error("Choose a file to delete");
  let label = "File";
  let removed = false;

  const unlinkMeta = (meta) => {
    if (meta && meta.stored_as) unlinkEnquiryStoredFile(raw.enquiry_no, meta.stored_as);
  };

  if (want === "quote" || /^quote_(\d+)$/.test(want)) {
    const n = want === "quote" ? 0 : Number(want.split("_").pop());
    const quotes = Array.isArray(raw.quotes) ? raw.quotes.slice() : [];
    const idx = quotes.findIndex((q) => {
      if (!q) return false;
      const qKind = (q.file && q.file.kind) || (q.n ? "quote_" + q.n : "");
      if (qKind && qKind === want) return true;
      if (want === "quote" && (!qKind || qKind === "quote")) return true;
      return n && Number(q.n) === n;
    });
    if (idx >= 0) {
      const hit = quotes[idx];
      label = (hit.quote_no ? "Quote " + hit.quote_no : "Quote") + (hit.option ? " option " + hit.option : "");
      unlinkMeta(hit.file);
      quotes.splice(idx, 1);
      raw.quotes = quotes;
      if (String(raw.chosen_quote_no || "") && hit.quote_no && String(raw.chosen_quote_no) === String(hit.quote_no)) {
        raw.chosen_quote_no = "";
        raw.chosen_option = "";
      }
      removed = true;
    } else if (want === "quote" && enquiryHasQuotePdf(raw.enquiry_no)) {
      label = "Quote PDF" + (raw.quote_no ? " " + raw.quote_no : "");
      removeEnquiryQuotePdf(raw.enquiry_no);
      raw.quote_pdf_name = "";
      raw.quote_pdf_uploaded_at = "";
      if (!quotes.length) {
        raw.quote_no = "";
        raw.date_quoted = "";
      }
    ...
    if (removed) refreshCanonicalQuotePdf(raw);
  } else if (want === "cost_sheet" || /^cost_sheet_/.test(want)) {
    const sheets = Array.isArray(raw.cost_sheets) && raw.cost_sheets.length
      ? raw.cost_sheets.slice()
      : (raw.cost_sheet && raw.cost_sheet.stored_as ? [raw.cost_sheet] : []);
    const kept = [];
    sheets.forEach((sheet) => {
      const sheetKind = (sheet && sheet.kind) || "cost_sheet";
      if (sheetKind === want || (want === "cost_sheet" && (!sheet.kind || sheet.kind === "cost_sheet"))) {
        label = sheet.product ? ("Cost sheet · " + sheet.product) : "Cost sheet";
        unlinkMeta(sheet);
        removed = true;
      } else kept.push(sheet);
    });
    raw.cost_sheets = kept;
    raw.cost_sheet = kept.length ? kept[kept.length - 1] : null;
  } else if (want === "pop") {
    const file = raw.client_outcome && raw.client_outcome.file;
    if (file && file.stored_as) {
      label = "Proof of payment";
      unlinkMeta(file);
      raw.client_outcome = Object.assign({}, raw.client_outcome, { file: null });
      removed = true;
    }
  } else if (want === "drawing") {
    const file = raw.drawing && raw.drawing.file;
    if (file && file.stored_as) {
      label = "Drawing";
      unlinkMeta(file);
      raw.drawing = Object.assign({}, raw.drawing, { file: null });
      removed = true;
    }
  } else if (want === "follow_up" || /^follow_up_(\d+)$/.test(want)) {
    const list = Array.isArray(raw.follow_ups) ? raw.follow_ups.slice() : [];
    const kept = [];
    list.forEach((item, i) => {
      const file = item && item.file;
      const itemKind = (file && file.kind) || ("follow_up_" + (item.n || i + 1));
      const isLast = i === list.length - 1;
      if (itemKind === want || (want === "follow_up" && isLast)) {
    ...
    raw.follow_ups = kept;
  } else if (want === "supplier" || /^supplier_(\d+)$/.test(want)) {
    const n = want === "supplier" ? 0 : Number(want.split("_").pop());
    const list = Array.isArray(raw.supplier_quotes) ? raw.supplier_quotes.slice() : [];
    const kept = [];
    list.forEach((item, i) => {
      const file = item && item.file;
      const itemKind = (file && file.kind) || ("supplier_" + (item.n || i + 1));
      const isLast = i === list.length - 1;
      if (itemKind === want || (want === "supplier" && isLast) || (n && Number(item.n) === n)) {
        label = "Supplier quotation";
        unlinkMeta(file);
        removed = true;
      } else kept.push(item);
    });
    raw.supplier_quotes = kept;
  } else if (/^correspondence_(\d+)$/.test(want) || want === "correspondence") {
    const corr = normalizeCorrespondence(raw);
    const mails = (corr.mails || []).slice();
    const files = (corr.files || []).slice();
    const keepMails = [];
    mails.forEach((mail, i) => {
      const mailKind = (mail && mail.kind) || ("correspondence_" + (i + 1));
      if (mailKind === want || (want === "correspondence" && i === 0)) {
        label = "Correspondance link";
        unlinkMeta(mail);
        removed = true;
      } else keepMails.push(mail);
    });
    const keepFiles = [];
    files.forEach((file, i) => {
      const fileKind = (file && file.kind) || ("correspondence_" + (i + 1));
      if (fileKind === want) {
        label = "Correspondance";
        unlinkMeta(file);
        removed = true;
    ...
    raw.correspondence = { mails: keepMails, files: keepFiles };
  }

  if (!removed) throw new Error("That file was not found on this enquiry");
  appendEnquiryEvent(raw, {
    kind: "delete_file",
    actor: "",
    status: raw.status || "",
    label: "Deleted " + label,
  ...
  save();
  return decorateEnquiry(raw);
}

function deleteEnquiry(enquiryNo) {
  const n = enquiryNumberValue(enquiryNo);
  const row = n
    ? (state.enquiries || []).find((o) => enquiryNumberValue(o.enquiry_no) === n)
    : null;
  if (!row) throw new Error("Enquiry not found");
  const want = row.enquiry_no;
  try { removeEnquiryQuotePdf(want); } catch (e) {}
  try { removeEnquiryFiles(want); } catch (e) {}
  state.enquiries = (state.enquiries || []).filter((o) => enquiryNumberValue(o.enquiry_no) !== n);
  save();
  return { enquiry_no: want };
}
